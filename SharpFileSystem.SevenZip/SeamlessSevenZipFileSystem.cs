    ...
    public class SeamlessSevenZipFileSystem : SeamlessArchiveFileSystem
    {
        ...
        protected override bool IsArchiveFile(IFileSystem fileSystem, FileSystemPath path)
        {
            return path.IsFile
                   ...
                   &&
                   !HasArchive(
                       path) // HACK: Disable ability to open archives inside archives (SevenZip's stream does not have the ability to trace at the moment).
                ;
        }
        ...
        protected override IFileSystem CreateArchiveFileSystem(File archiveFile)
        {
            ...
            {
                archiveFs = new SevenZipFileSystem(
                    ((PhysicalFileSystem) archiveFile.FileSystem).GetPhysicalPath(archiveFile.Path));
            }
            else
            {
                var archiveStream = archiveFile.FileSystem.OpenFile(archiveFile.Path, FileAccess.Read);
