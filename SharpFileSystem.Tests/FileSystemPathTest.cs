...
namespace SharpFileSystem.Tests
{
    ...
    {
        private readonly FileSystemPath[] _paths =
        {
            root,
            directoryA,
            fileA,
        ...
        private IEnumerable<FileSystemPath> Directories
        {
            get { return _paths.Where(p => p.IsDirectory); }
        }

        private IEnumerable<FileSystemPath> Files
        {
            get { return _paths.Where(p => p.IsFile); }
        }
        ...
        public void CompareToTest()
        {
            ...
            foreach (var pb in _paths)
                Assert.Equal(Math.Sign(pa.CompareTo(pb)),
                    Math.Sign(string.Compare(pa.ToString(), pb.ToString(), StringComparison.Ordinal)));
        }
        ...
        {
            Assert.Equal(fileA.EntityName, "filea");
            Assert.Equal(fileB.EntityName, "fileb.txt");
            Assert.Equal(root.EntityName, null);
        }
        ...
        {
            Assert.Equal(0, root.GetDirectorySegments().Length);
            Directories
                .Where(d => !d.IsRoot)
        ...
        public void GetExtensionTest()
        {
            ...
            Assert.Equal(fileB.GetExtension(), ".txt");
